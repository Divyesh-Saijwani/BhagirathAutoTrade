...
namespace BhagirathAutoTrade.Server.Services
{
    public class EquityService : IEquityService
    {
        ...
        public async Task<string> GetCloseDataAsync(string workingDate, string expiryDate, string exchange, string instrument, string optionType, string type, string strikePrice, string symbol)
        {
            try
            {
                var url = $"GetCloseData?workingDate={workingDate}&expiryDate={expiryDate}&exchange={exchange}&instrument={instrument}&optionType={optionType}&type={type}&strickPrice={strikePrice}&symbole={symbol}";
        ...
        public async Task<IEnumerable<string>> GetExpiryDateAsync(string symbol, string optionType)
        {
            try
            {
                ...
                var isFuture=optionType.ToLower().Equals("xx")?true:false;
                switch (symbol.ToLower())
                {
                    case "nifty":
                        dates = isFuture ? DateTime.Now.GetLastDayOfMonth(DayOfWeek.Thursday): DateTime.Now.GetNextDayOfWeek(DayOfWeek.Thursday);
                        break;

                    case "banknifty":
                        dates = isFuture ? DateTime.Now.GetLastDayOfMonth(DayOfWeek.Wednesday) : DateTime.Now.GetNextDayOfWeek(DayOfWeek.Wednesday);
                        break;

                    case "finnifty":
                        dates = isFuture ? DateTime.Now.GetLastDayOfMonth(DayOfWeek.Tuesday) : DateTime.Now.GetNextDayOfWeek(DayOfWeek.Tuesday);
                        break;
                }
