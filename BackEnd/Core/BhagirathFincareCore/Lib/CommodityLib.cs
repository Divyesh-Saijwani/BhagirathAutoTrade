...
namespace BhagirathFincareCore.Lib
{
    public class CommodityLib
    {
        ...
        public object CalculateCloseForCommodity(DateTime workingDate, DateTime expiryDate, string exchange, string symbole, string instrument, string optionType, string strickPrice)
        {
            ...
            try
            {
                ...
                if (exchange == "MCX")
                {
                            ...
                            closeData =
                                db.MarketPrice.Where(
                                    x =>
                                        ...
                                        x.OptionType == optionType && x.FileType == 1 && x.StrikePrice == strickPrice &&
                                        x.Symbol == symbole)
                    ...
                    else
                    {
                        ...
                        //    new SqlParameter("@StrikePrice", "0.00")
                        //    };
                        ...
                        BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                        {
                            closeData =
                                db.MarketPrice.Where(
                                    x =>
                                        x.UploadDate < workingDate && x.ExpiryDate == expiryDate &&
                                        x.OptionType == "xx" && x.FileType == fileType && x.StrikePrice == "0.00" &&
                                        x.Symbol == symbole)
                                    .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                    .FirstOrDefault();
                        }
            ...
            {
                ex.LogError(this);
                closeData = "We are not able to get results for this case";
                return null;
            }
            return closeData;
        }

        public object CalculateOpenForCommodity(DateTime workingDate, DateTime expiryDate, string exchange, string symbole, string instrument, string optionType, string strickPrice)
        {
            ...
            {
                ex.LogError(this);
                openData = "We are not able to get results for this case";
                return null;
            }
            return openData;
        }
        public object AutoCompleteCompanyForCommodity(string exchange)
        {
            try
            {
                ...
                int fileType = 1;
                if (exchange.Equals("MCX", StringComparison.OrdinalIgnoreCase))
                    fileType = 1;
                else if (exchange.Equals("NCDEX", StringComparison.OrdinalIgnoreCase))
                    fileType = 3;

                sqlParams = new SqlParameter[]
                           {
                           new SqlParameter { ParameterName = "@filetype",  Value =fileType, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                ...
                //companyList = db.MarketPrice.Where(x => x.FileType == fileType).OrderBy(x => x.Symbol).Select(x => x.Symbol).Distinct().ToList();
                //return companyList;
                //return data;
            }
            catch (Exception ex)
        ...
        //public object GetExpireDateBySymbolName(string symbol, string exchange, string optiontype)
        public List<DateTime?> GetExpireDateBySymbolName(string symbol, string exchange, string optiontype)
        {
            try
            {
                ...
                if (exchange == "MCX")
                {
                    ...
                    optiontype = string.IsNullOrEmpty(optiontype) ? "XX" : optiontype;
                    expireDate =
                                   db.MarketPrice.Where(x => x.Symbol.ToLower() == symbol.ToLower().Trim() && x.FileType == 1 && x.OptionType == optiontype)
                ...
                {
                    expireDate =
                   db.MarketPrice.Where(x => x.Symbol == symbol && x.FileType == 3)
                       .OrderByDescending(x => x.ExpiryDate)
                       .Select(x => x.ExpiryDate).Distinct()
                       .ToList();
                }
        ...
        public object GetStrikePrice(string exchange, string symbol, DateTime expirydate, string optiontype)
        {
            try
            {
                BhagirathFinCareEntities db = new BhagirathFinCareEntities();

                List<MarketPrice> strickPrice = new List<MarketPrice>();
                strickPrice = db.MarketPrice.Where(x => x.Symbol.Equals(symbol.Trim()) && x.FileType == 1 && x.OptionType.Equals(optiontype) && x.ExpiryDate == expirydate && x.StrikePrice.Trim() != "0")
                    .GroupBy(x => x.StrikePrice).Select(x => x.FirstOrDefault()).OrderBy(x => x.StrikePrice).ToList();

                //strickPrice = db.MarketPrice.Where(x => x.Symbol.Equals(symbol) &&  x.FileType == '1' && x.StrikePrice.Trim() != "0" && !string.IsNullOrEmpty(x.StrikePrice.Trim()))
                //    .Distinct().ToList().OrderBy(x => Convert.ToInt32(x.StrikePrice.Trim()))
                //    .Select(x => x.StrikePrice).Distinct().ToList();
                return strickPrice;
            }
            catch (Exception ex)
            {
        ...
        public object CalculateCommodity(DtoRequestModelForCalculate dtoCalculate, out string message)
        {
            ...
            try
            {
                if (dtoCalculate != null)
                {
                    try
                    {
                        ...
                        multiGainer = new Multigainer()
                        {
                            ...
                            strikeprice = dtoCalculate.StrickPrice,
                            Companyname = dtoCalculate.Symbole
                            //strikeprice=dtoCalculate.StrickPrice
                        };

                        string Companyname = string.Empty;
                        if (multiGainer.Companyname != null)
                        {
                            Companyname = multiGainer.Companyname.ToString() + "";
                        }
                        double strikeprice = 0;
                        if (multiGainer.strikeprice != null && multiGainer.strikeprice != "")
                        {
                        ...
                        //string type = Request.Form["Typedropdown"].ToString();                    
                        double IDH = Convert.ToDouble(multiGainer.txt_t10);
                        double idl = Convert.ToDouble(multiGainer.txt_d10);
                        ...
                        {
                            case "mcx":
                                fileType = FileTypeEnum.MCX;
                                break;
                            case "ncdex":
                                fileType = FileTypeEnum.NCDEX;
                                break;
                        }

                        decimal open = 0, close = 0;

                        //var pdlpdhData = GetPDLPDHDataForCommodity(multiGainer, Decimal.TryParse(multiGainer.txt_m4, out open) ? open : 0, Decimal.TryParse(multiGainer.txt_m5, out close) ? close : 0, Convert.ToDateTime(multiGainer.WorkingDate), Companyname, (int)fileType);
                        //if (pdlpdhData != null)
                        //{
                        //    multiGainer.txt_d11 = pdlpdhData.PDL.ToString();
                        //    multiGainer.txt_t11 = pdlpdhData.PDH.ToString();

                        //    var tdltdhData = GetTDLTDHDataForCommodity(multiGainer, pdlpdhData.PDL, pdlpdhData.PDH, pdlpdhData.SelectedDateForPDHAndPDL, Companyname, (int)fileType);
                        //    if (tdltdhData != null)
                        //    {
                        ...
                        //    var cdlcdhData = GetCTLDCTHDDataForCommodity(multiGainer, (decimal)idl, (decimal)IDH, Convert.ToDateTime(multiGainer.WorkingDate, CultureInfo.InvariantCulture), Companyname, (int)fileType);
                ...
                Recalculate:
                    ...
                    if (l10 == e19) // if ct = CTLD
                    {
                        ...
                        multiGainer.txt_K9 = arrwr[Math.Abs(myIndex - 1)].ToString();  // calculated ws
                        k9 = arrwr[Math.Abs(myIndex - 1)];
                        // wr level  should be above bnr level from tempws above cthd
                        // bnr = m8
                        myObject = m8; //bnr
        ...
        public object getCalculateDataForCommodity(string Instrument, string symbol, string exchange, DateTime workingdate, DateTime expirydate, string optionType)
        {
            try
            {
                ...
                {
                    optionType = string.IsNullOrEmpty(optionType) ? "XX" : optionType;
                    var GetData = context.RecursiveUpload.Where(x => x.Symbol == symbol && x.FileType == 1 && EntityFunctions.TruncateTime(x.UploadDateTime) == EntityFunctions.TruncateTime(WorkingDate) && EntityFunctions.TruncateTime(x.ExpiryDate) == EntityFunctions.TruncateTime(expirydate) && x.OptionType == optionType).OrderByDescending(x => x.FileNo).FirstOrDefault();
                    if (GetData != null)
                    {
                        return GetData;
                    }
                ...
                {
                    optionType = optionType == null ? String.Empty : optionType;
                    var GetData = context.RecursiveUpload.Where(x => x.Symbol == symbol && x.FileType == 1 && EntityFunctions.TruncateTime(x.UploadDateTime) == EntityFunctions.TruncateTime(WorkingDate) && EntityFunctions.TruncateTime(x.ExpiryDate) == EntityFunctions.TruncateTime(expirydate) && x.OptionType == optionType).OrderByDescending(x => x.FileNo).FirstOrDefault();
                    if (GetData != null)
                    {
                        return GetData;
        ...
        #region Commodity - Calculate PDH-PDL, TDH-TDL, CTHD-CTLD
            ...
            int fileType)
        {
            ...
            try
            {
                ...
                if (dtoCalculate.Instrument.Equals("FUTCOM", StringComparison.OrdinalIgnoreCase))
                {
                    sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} and OptionType='XX' and StrikePrice='0.00' and ExpiryDate='{3}' order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType, dtoCalculate.ExpiryDate);
                    //SqlParameter[] paramList = new SqlParameter[] {
                ...
                else if (dtoCalculate.Instrument.Equals("OPTFUT", StringComparison.OrdinalIgnoreCase))
                {
        ...
        public TDLTDHSelectedData GetTDLTDHData(DtoRequestModelForCalculate dtoCalculate, decimal pdl, decimal pdh, DateTime pdlpdhDate, string companyName, int fileType)
        {
            ...
            try
            {
                ...
                if (dtoCalculate.Instrument.Equals("FUTCOM", StringComparison.OrdinalIgnoreCase))
                {
                    sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} and OptionType='XX' and StrikePrice='0.00' and ExpiryDate='{3}' order by [UploadDate] desc", companyName, pdlpdhDate.ToString("yyyy-MM-dd"), fileType, dtoCalculate.ExpiryDate);
                    //SqlParameter[] paramList = new SqlParameter[] {
                    //        new SqlParameter("@ExpiryDate", DateTime.ParseExact(dtoCalculate.ExpiryDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
        ...
        //public PDLPDHSelectedData GetPDLPDHDataForCommodity(Multigainer multiGainer, decimal currentOpen, decimal currentClose, DateTime currentDate, string companyName,
        //    int fileType)
        //{
        //    PDLPDHSelectedData pdlpdhSelectedData = null;
        //    try
        ...
        //        SqlParameter[] paramList = new SqlParameter[] {
        //                    new SqlParameter("@ExpiryDate", DateTime.ParseExact(multiGainer.expirydate ,"MM/dd/yyyy", new CultureInfo("en-GB"))),
        //                    new SqlParameter("@WorkingDate", currentDate.ToString("yyyy-MM-dd")),
        //                    new SqlParameter("@OptionType", string.Empty),
        //                    new SqlParameter("@FileType", fileType),
        //                    new SqlParameter("@Symbol", companyName),
        //                    new SqlParameter("@StrikePrice", string.Empty)
        //                    };
        ...
        //public TDLTDHSelectedData GetTDLTDHDataForCommodity(Multigainer multiGainer, decimal pdl, decimal pdh, DateTime pdlpdhDate, string companyName, int fileType)
        //{
        //    TDLTDHSelectedData tdltdhSelectedData = null;
        //    try
        //    {
        ...
        //        SqlParameter[] paramList = new SqlParameter[] {
        //                    new SqlParameter("@ExpiryDate", DateTime.ParseExact(multiGainer.expirydate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
        //                    new SqlParameter("@WorkingDate", Convert.ToDateTime(multiGainer.WorkingDate).ToString("yyyy-MM-dd")),
        //                    new SqlParameter("@OptionType", ""),
        //                    new SqlParameter("@FileType", fileType),
        //                    new SqlParameter("@Symbol", companyName),
        //                    new SqlParameter("@StrikePrice", ""),
        ...
        //public CTLDCTHDSelectedData GetCTLDCTHDDataForCommodity(Multigainer multiGainer, decimal currentLow, decimal currentHigh, DateTime currentWorkingDate, string companyName, int fileType)
        //{
        //    CTLDCTHDSelectedData ctldcthdSelectedData = new CTLDCTHDSelectedData();
        //    ctldcthdSelectedData.CompanyName = companyName;
        //    ctldcthdSelectedData.FileType = fileType;
        ...
        //private DataTable objToDataTable(object obj)
        //{
        //    DataTable dt = new DataTable();
        //    MarketPrice objmkt = new MarketPrice();
        //    dt.Columns.Add("Column_Name");
        //    foreach (PropertyInfo info in typeof(MarketPrice).GetProperties())
        //    {
        ...
        public object getMstAndMsts(DtoRequestModelForCalculate dtoCalculate)
        {
            try
            {
                ...
                DateTime exp = DateTime.ParseExact(dtoCalculate.ExpiryDate, "MM/dd/yyyy",
                    new CultureInfo("en-US"),
                    DateTimeStyles.None);
                var uploadDateMonth = uploadDate.Month;
                var expMonth = exp.Month;
                var firstDayOfMonth = new DateTime(uploadDate.Year, uploadDate.Month, 1);
                var lastdayofmonth = new DateTime(exp.Year, exp.Month, 1).AddMonths(1).AddDays(-1).Date;
                Msts msts = new Msts();
                string Exchange = dtoCalculate.Exchange;
                string optiontype = dtoCalculate.OptionType;
                string strikeprice = dtoCalculate.StrickPrice;
                string Companyname = dtoCalculate.Symbole.Trim();
                string expirydate = dtoCalculate.ExpiryDate;
                string instrument = dtoCalculate.Instrument;
                List<DateTime?> expirylist = GetExpireDateBySymbolName(Companyname, Exchange, optiontype);//.GetType().GetProperties().ToArray();
                ...
                if (Exchange.ToLower() == "mcx")
                {
                    ...
                    if (instrument == "FUTCOM")
                    {
                        ...
                        #region 
                        //for (int i = 0; i < expArrayList.Count; i++)
                        //{
                        //    dict.Clear();
                        //    //DateTime expdate = DateTime.ParseExact(dtoCalculate.ExpiryDate, "dd-MM-yyyy",new CultureInfo("en-US"),DateTimeStyles.None);
                        //    if (Convert.ToDateTime(expArrayList[i])==Convert.ToDateTime(dtoCalculate.ExpiryDate))
                        //    {
                        //        if (uploadDateMonth == expMonth)
                        //        {
                        //            dict.Add(0, Convert.ToDateTime(expArrayList[i]));
                        //        }
                        //        else if (uploadDateMonth < expMonth)
                        //        {
                        //            if (expArrayList.Contains(dtoCalculate.ExpiryDate))
                        //            {
                        //                dict.Add(-1, Convert.ToDateTime(expArrayList[i - 1]));
                        //            }
                        //        }
                        //        else if (uploadDateMonth > expMonth)
                        //        {
                        ...
                        var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.UploadDate <= uploadDate && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.OptionType == "XX" && x.High != "").Select(x => x.High).ToList();
                        listDecimal = highlist.Select(decimal.Parse).ToList();
                        HH = listDecimal.Max();
                        var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.UploadDate <= uploadDate && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.OptionType == "XX" && x.Low != "0.00" && x.Low != "").Select(x => x.Low).ToList();
                        ...
                        msts.lmst = LMST;
                        msts.lmsts = LMSTS;
                        var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX" && x.Low != "" && x.High != "").Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                        var days = data.OrderByDescending(x => x.UploadDate).ToList();
                        if (msts.hmst != null && msts.lmst != null)
                        {
                            foreach (var items in days)
                            {
                                decimal idl = Convert.ToDecimal(items.Low);
                                decimal idh = Convert.ToDecimal(items.High);
                    ...
                    else if (instrument == "OPTFUT")
                    {
                        ...
                        DateTime one = (DateTime)dict[1];
                        DateTime minusone = (DateTime)dict[-1];
                        var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice && x.High != "").Select(x => x.High).ToList();
                        listDecimal = highlist.Select(decimal.Parse).ToList();
                        HH = listDecimal.Max();
                        var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice && x.Low != "").Select(x => x.Low).ToList();
                        listDecimal2 = lowlist.Select(decimal.Parse).ToList();
                        HL = listDecimal2.Min();
                        decimal difference = Math.Abs(Convert.ToDecimal(HH) - Convert.ToDecimal(HL));
                        decimal GR1 = difference * Convert.ToDecimal(0.62);
                        ...
                        msts.lmst = LMST;
                        msts.lmsts = LMSTS;
                        var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice).Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                        var days = data.OrderByDescending(x => x.UploadDate).ToList();
                        if (msts.hmst != null && msts.lmst != null)
                        {
                ...
                else
                {
                    List<decimal> listDecimal = new List<decimal>();
                    List<decimal> listDecimal2 = new List<decimal>();
                    var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.ExpiryDate == exp && x.UploadDate <= uploadDate && x.High != "").Select(x => x.High).ToList();
                    listDecimal = highlist.Select(decimal.Parse).ToList();
                    HH = listDecimal.Max();
                    var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.ExpiryDate == exp && x.UploadDate <= uploadDate && x.Low != "").Select(x => x.Low).ToList();
                    listDecimal2 = lowlist.Select(decimal.Parse).ToList();
                    HL = listDecimal2.Min();
                    decimal difference = Math.Abs(Convert.ToDecimal(HH) - Convert.ToDecimal(HL));
                    decimal GR1 = difference * Convert.ToDecimal(0.62);
                    ...
                    msts.lmst = LMST;
                    msts.lmsts = LMSTS;
                    var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.ExpiryDate == exp && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice).Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                    var days = data.OrderByDescending(x => x.UploadDate).ToList();
                    if (msts.hmst != null && msts.lmst != null)
                    {
                        foreach (var item in days)
