...
namespace BhagirathFincareCore.Lib
{
    public class CurrencyLib
    {
        ...
        public object AutoCompleteCompanyForCurrency(string exchange, string instrument, string optionType)
        {
            try
            {
                string sqlQuery;
                ...
                if (exchange == "BSE")
                {
                    if (instrument.Equals("FUTCUR", StringComparison.OrdinalIgnoreCase) ||
                        instrument.Equals("FUTIRD", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                  {
                         new SqlParameter { ParameterName = "@filetype",  Value =7 , Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 7).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                    else
                    {
                        sqlParams = new SqlParameter[]
                  {
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 7
                        //&& x.OptionType == optionType).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                }
                else if (exchange == "MCX-SX")
                {
                    if (instrument.Equals("FUTCUR", StringComparison.OrdinalIgnoreCase) ||
                        instrument.Equals("FUTIRF", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                  {
                         new SqlParameter { ParameterName = "@filetype",  Value =2, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                         new SqlParameter { ParameterName = "@optiontype",  Value ="", Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                  };
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 2).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                    else
                    {
                        sqlParams = new SqlParameter[]
                  {
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 2
                        //&& x.OptionType == optionType).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                }
                else if (exchange == "NSE")
                {
                    if (instrument.Equals("FUTURE", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                  {
                         new SqlParameter { ParameterName = "@filetype",  Value =9, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                         new SqlParameter { ParameterName = "@optiontype",  Value ="", Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                  };
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 9).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                    else if (instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                  {
                         new SqlParameter { ParameterName = "@filetype",  Value =10, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                         new SqlParameter { ParameterName = "@optiontype",  Value =optionType, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                  };
                        ...
                        //marketPrices =
                        //db.MarketPrice.Where(x => x.FileType == 10
                        //&& x.OptionType == optionType).OrderBy(x => x.Symbol).ToList().Select(x => x.Symbol).Distinct().ToList();
                    }
                }
        ...
        public List<DateTime?> GetExpireDateBySymbolNameForCurrency(string exchange, string symbol, string instrument, string optionType)
        {
            try
            {
                ...
                {
                    case "bse":
                        fileType = FileTypeEnum.CURRENCYBSE;
                        break;
                    case "mcx-sx":
                        fileType = FileTypeEnum.CURRENCYMCXSX;
                        break;
                    case "nse":
                        if (instrument.Equals("FUTURE", StringComparison.OrdinalIgnoreCase))
                            fileType = FileTypeEnum.CURRENCYNSEFUTURE;
                        else if (instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                            fileType = FileTypeEnum.CURRENCYNSEOPTION;
                        break;
                }

                if (instrument.Equals("OPTCUR", StringComparison.OrdinalIgnoreCase) ||
                   instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                {
                    BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                    expireDate =
                        db.MarketPrice.Where(x => x.Symbol == symbol && x.OptionType == optionType && x.FileType == (int)fileType)
                ...
                else
                {
                    BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                    expireDate =
                        db.MarketPrice.Where(x => x.Symbol == symbol && x.FileType == (int)fileType)
                            .OrderByDescending(x => x.ExpiryDate)
                            .Select(x => x.ExpiryDate).Distinct()
                            .ToList();
                }
        ...
        public object GetStrikePriceForCurrency(string symbol, DateTime expireDate, string exchange, string instrument)
        {
            try
            {
                BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                List<string> strickPrice = new List<string>();
                if (exchange == "BSE")
                {
                    strickPrice = db.MarketPrice.Where(x => x.ExpiryDate == expireDate && x.FileType == 7 && x.Symbol.Contains(symbol))
                    .OrderBy(x => x.StrikePrice)
                    .Select(x => x.StrikePrice).Distinct().ToList();
                }
                else if (exchange == "MCX-SX")
                {
                    strickPrice = db.MarketPrice.Where(x => x.ExpiryDate == expireDate && x.FileType == 2 && x.Symbol.Contains(symbol))
                    .OrderBy(x => x.StrikePrice)
                    .Select(x => x.StrikePrice).Distinct().ToList();
                }
                else if (exchange == "NSE")
                {
                    if (instrument.Equals("FUTURE", StringComparison.OrdinalIgnoreCase))
                    {
                        strickPrice = db.MarketPrice.Where(x => x.ExpiryDate == expireDate && x.FileType == 9 && x.Symbol.Contains(symbol))
                        .OrderBy(x => x.StrikePrice)
                        .Select(x => x.StrikePrice).Distinct().ToList();
                    }
                    else if (instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                    {
                        strickPrice = db.MarketPrice.Where(x => x.ExpiryDate == expireDate && x.FileType == 10 && x.Symbol.Contains(symbol))
                        .OrderBy(x => x.StrikePrice)
                        .Select(x => x.StrikePrice).Distinct().ToList();
                    }
                }
        ...
        public object GetCloseDataForCurrency(DateTime workingDate, DateTime expiryDate, string exchange, string instrument, string symbole, string strickPrice, string optionType)
        {
            ...
            try
            {
                ...
                if (exchange == "BSE")
                {
                    ...
                    closeData =
                                ...
                                x => x.UploadDate < workingDate && x.FileType == 7 &&
                                x.Symbol == symbole)
                                .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                .FirstOrDefault();
                }
                else if (exchange.Equals("MCX-SX", StringComparison.OrdinalIgnoreCase) ||
                         exchange.Equals("NSE", StringComparison.OrdinalIgnoreCase))
                {
                    int fileType = 2;
                    if (exchange.Equals("MCX-SX", StringComparison.OrdinalIgnoreCase))
                        fileType = 2;
                    else if (exchange.Equals("NSE", StringComparison.OrdinalIgnoreCase))
                    {
                        if (instrument.Equals("FUTURE", StringComparison.OrdinalIgnoreCase))
                            fileType = 9;
                        else if (instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                    ...
                    SqlParameter[] paramList = new SqlParameter[] {
                            ...
                            new SqlParameter("@OptionType", string.IsNullOrEmpty(optionType)?"":optionType),
                            new SqlParameter("@FileType", fileType),
                            new SqlParameter("@Symbol", symbole),
                            new SqlParameter("@StrikePrice", string.IsNullOrEmpty(strickPrice)?"": strickPrice)
                            };
        ...
        public object GetOpenDataForCurrency(DateTime workingDate, DateTime expiryDate, string exchange, string instrument, string symbole, string strickPrice, string optionType)
        {
        ...
        public object CalculateCurrency(DtoRequestModelForCalculate dtoCalculate, out string message)
        {
            try
            {
                ...
                if (dtoCalculate != null)
                {
                    ...
                    try
                    {
                        ...
                        //string Companyname = multiGainer.companynamemv.ToString();


                        //string company = GetSymbolNameFromScriptCode(dtoCalculate.Symbole);
                        ...
                        //  return Json(multiGainer); // this is temperory once to we get value then further step wll be followed

                        if (dtoCalculate.OptionType == null)
                            dtoCalculate.OptionType = string.Empty;

                        if (dtoCalculate.StrickPrice == null)
                            dtoCalculate.StrickPrice = string.Empty;
                        ...
                        switch (Exchange.ToLower())
                        {
                            ...
                            case "mcx-sx":
                                ...
                                break;
                            case "nse":
                                if (dtoCalculate.Instrument.Equals("FUTURE", StringComparison.OrdinalIgnoreCase))
                                    fileType = FileTypeEnum.CURRENCYNSEFUTURE;
                                else if (dtoCalculate.Instrument.Equals("OPTION", StringComparison.OrdinalIgnoreCase))
                                    fileType = FileTypeEnum.CURRENCYNSEOPTION;
                                break;
                        }
                        ...
                        var pdlpdhData = GetPDLPDHDataForCurrency(dtoCalculate, dtoCalculate.Open, dtoCalculate.Close, Convert.ToDateTime(dtoCalculate.WorkingDate, CultureInfo.InvariantCulture), Companyname, (int)fileType);
                        if (pdlpdhData != null)
                        {
                            multiGainer.txt_d11 = pdlpdhData.PDL.ToString();
                            multiGainer.txt_t11 = pdlpdhData.PDH.ToString();

                            var tdltdhData = GetTDLTDHDataForCurrency(dtoCalculate, pdlpdhData.PDL, pdlpdhData.PDH, pdlpdhData.SelectedDateForPDHAndPDL, Companyname, (int)fileType);
                            if (tdltdhData != null)
                            {
                            ...
                            var cdlcdhData = GetCTLDCTHDDataForCurrency(dtoCalculate, (decimal)idl, (decimal)IDH, Convert.ToDateTime(dtoCalculate.WorkingDate, CultureInfo.InvariantCulture), Companyname, (int)fileType);
                    ...
                    k13 = "";
                    //string passdate = multiGainer.WorkingDate.ToString();

                    // call the stored procedure here for sunil mistry
                    // the stored procedure parameter is @companyname which u need to select from dropdownlist
                    // and passeddte which is the date selection box on screen 
                    // pass this as input parameter to stored procedure name mv9_cthd_ctld
                    //and take six output  value into 
                    // f19 = ctld
                    //q16 = cthd
                    //d11 = pdl
                    //t11= pdh
                    ...
                    #endregion
                    //b12 calculatrion
                    //b12 = (t11 - d11) * 100 / m3;  // previusly it was m3
                    //b12 = (t11 - d11) * 100 / m5; commented on 17april2014
                    b12 = (t11 - d11) * 100 / m3;
        ...
        #endregion
        #region Currency - Calculate PDH-PDL, TDH-TDL, CTHD-CTLD
        public PDLPDHSelectedData GetPDLPDHDataForCurrency(DtoRequestModelForCalculate dtoCalculate, decimal currentOpen, decimal currentClose, DateTime currentDate, string companyName,
            int fileType)
        {
            ...
            try
            {
                ...
                SqlParameter[] paramList = new SqlParameter[] {
                            ...
                            new SqlParameter("@OptionType", dtoCalculate.OptionType),
                            new SqlParameter("@FileType", fileType),
                            new SqlParameter("@Symbol", companyName),
                            new SqlParameter("@StrikePrice", dtoCalculate.StrickPrice)
                            };
        ...
        public TDLTDHSelectedData GetTDLTDHDataForCurrency(DtoRequestModelForCalculate dtoCalculate, decimal pdl, decimal pdh, DateTime pdlpdhDate, string companyName, int fileType)
        {
            ...
            try
            {
                ...
                SqlParameter[] paramList = new SqlParameter[] {
                            new SqlParameter("@ExpiryDate", DateTime.ParseExact(dtoCalculate.ExpiryDate ,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            new SqlParameter("@WorkingDate", Convert.ToDateTime(dtoCalculate.WorkingDate,CultureInfo.InvariantCulture)),
                            new SqlParameter("@OptionType", dtoCalculate.OptionType),
                            new SqlParameter("@FileType", fileType),
                            new SqlParameter("@Symbol", companyName),
                            new SqlParameter("@StrikePrice", dtoCalculate.StrickPrice),
        ...
        public CTLDCTHDSelectedData GetCTLDCTHDDataForCurrency(DtoRequestModelForCalculate dtoCalculate, decimal currentLow, decimal currentHigh, DateTime currentWorkingDate, string companyName, int fileType)
        {
        ...
        #region calculate Msts and Msts
        public object getMstAndMsts(DtoRequestModelForCalculate dtoCalculate)
        {
            ...
            try
            {
                ...
                if (dtoCalculate != null)
                {
                    ...
                    string Companyname = dtoCalculate.Symbole;
                    List<DateTime?> expirylist = GetExpireDateBySymbolNameForCurrency(Exchange, Companyname, instrument, optiontype);//.GetType().GetProperties().ToArray();
                    ...
                    if (Exchange == "nse")
                    {
                        if (instrument == "FUTURE")
                        {
                            ...
                            var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate).Select(x => x.High).ToList();
                            listdecimal = highlist.Select(decimal.Parse).ToList();
                            HH = listdecimal.Max();
                            var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate).Select(x => x.Low).ToList();
                            listdecimal2 = lowlist.Select(decimal.Parse).ToList();
                            HL = listdecimal2.Min();
                            var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate).Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                        ...
                        else
                        {
                            ...
                            DateTime one = (DateTime)dict[1];
                            DateTime minusone = (DateTime)dict[-1];
                            var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice).Select(x => x.High).ToList();
                            listdecimal = highlist.Select(decimal.Parse).ToList();
                            HH = listdecimal.Max();
                            var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice).Select(x => x.Low).ToList();
                            listdecimal2 = lowlist.Select(decimal.Parse).ToList();
                            HL = listdecimal2.Min();
                    ...
                    if (Exchange == "bse")
                    {
                        ...
                        else
                        {
                            ...
                            DateTime one = (DateTime)dict[1];
                            DateTime minusone = (DateTime)dict[-1];
                            var highlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX").Select(x => x.High).ToList();
                            listdecimal = highlist.Select(decimal.Parse).ToList();
                            HH = listdecimal.Max();
                            var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX").Select(x => x.Low).ToList();
                            listdecimal2 = lowlist.Select(decimal.Parse).ToList();
                            HL = listdecimal2.Min();
                            decimal difference = Math.Abs(Convert.ToDecimal(HH) - Convert.ToDecimal(HL));
                            decimal GR1 = difference * Convert.ToDecimal(0.62);
                            ...
                            msts.lmst = LMST;
                            msts.lmsts = LMSTS;
                            var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX").ToList();
                            var days = data.OrderByDescending(x => x.UploadDate).ToList();
                            if (msts.hmst != null && msts.lmst != null)
                            {
                                foreach (var item in days)
                    ...
                    if (Exchange.ToLower() == "MCX-SX")
                    {
                        if (instrument == "OPTCUR")
                        {
                            ...
                            msts.lmst = LMST;
                            msts.lmsts = LMSTS;
                            var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == optiontype && x.StrikePrice == strikeprice).ToList();
                            var days = data.OrderByDescending(x => x.UploadDate).ToList();
                            if (msts.hmst != null && msts.lmst != null)
                            {
                                foreach (var item in days)
