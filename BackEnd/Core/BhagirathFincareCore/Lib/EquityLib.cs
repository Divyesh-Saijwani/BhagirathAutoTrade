...
namespace BhagirathFincareCore.Lib
{
    public class EquityLib
    {
        ...
        private void connection()
        {
            try
            {
                sqlconn = "Data Source=bhagirath.database.windows.net;Initial Catalog=BhagirathAutoTrade;User ID=BhagirathUser;Password=Bhagirath@123";
        ...
        public object AutoCompleteCompanyForEquity(string exchange, string type, string instrument, string optionType)
        {
            try
            {
                ...
                sqlQuery = "AutoCompleteSymbol @filetype, @optiontype";
                ...
                if (exchange == "BSE" && type == "EQ")
                {
                   ...
                   {
                         new SqlParameter { ParameterName = "@filetype",  Value =5 , Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                         new SqlParameter { ParameterName = "@optiontype",  Value ="", Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                   };

                    using (BhagirathFinCareEntities dbcontext = new BhagirathFinCareEntities())
                    {
                        data = dbcontext.Database.SqlQuery<string>(sqlQuery, sqlParams).ToList();
                ...
                //marketPrices =
                //        db.MarketPrice.Where(x => x.FileType == 5)
                //            .OrderBy(x => x.Symbol).Select(x => x.ScriptCode).Distinct()
                //            //.Take(50)
                //            .ToList();
                else if (exchange == "BSE" && type == "DERIVATIVE")
                {
                    if (instrument.Equals("OPTIDX", StringComparison.OrdinalIgnoreCase) ||
                        instrument.Equals("OPTSTK", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                        {
                           new SqlParameter { ParameterName = "@filetype",  Value =6, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                           new SqlParameter { ParameterName = "@optiontype",  Value =optionType, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                        };
                        ...
                        //marketPrices =
                        //   db.MarketPrice.Where(x => x.FileType == 6 && x.OptionType.Equals(optionType, StringComparison.OrdinalIgnoreCase))
                    ...
                    else
                    {
                        ...
                        //marketPrices =
                        //   db.MarketPrice.Where(x => x.FileType == 6)
                        //       .OrderBy(x => x.Symbol).Select(x => x.Symbol).Distinct()
                        //       //.Take(50)
                        //       .ToList();
                    }
                ...
                else if (exchange == "NSE" && type == "EQ")
                {
                    sqlParams = new SqlParameter[]
                        {
                           new SqlParameter { ParameterName = "@filetype",  Value =4, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                           new SqlParameter { ParameterName = "@optiontype",  Value ="", Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                        };
                    ...
                    //marketPrices =
                    //    db.MarketPrice.Where(x => x.FileType == 4)
                    //        .OrderBy(x => x.Symbol).Select(x => x.Symbol).Distinct()
                    //        //.Take(50)
                    //        .ToList();
                }
                ...
                else if (exchange == "NSE" && type == "DERIVATIVE")
                {
                    if (instrument.Equals("OPTIDX", StringComparison.OrdinalIgnoreCase) ||
                        instrument.Equals("OPTSTK", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlParams = new SqlParameter[]
                        {
                           new SqlParameter { ParameterName = "@filetype",  Value =11, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                           new SqlParameter { ParameterName = "@optiontype",  Value =optionType.ToLower(), Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                        };
                        ...
                        //marketPrices =
                        //   db.MarketPrice.Where(x => x.FileType == 8 && x.OptionType.Equals(optionType, StringComparison.OrdinalIgnoreCase))
                        //       .OrderBy(x => x.Symbol).Select(x => x.Symbol).Distinct()
                        //       //.Take(50)
                        //       .ToList();
                    }
                    else
                    {
                        sqlParams = new SqlParameter[]
                        {
                           new SqlParameter { ParameterName = "@filetype",  Value =8, Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.Int},
                           new SqlParameter { ParameterName = "@optiontype",  Value ="", Direction = System.Data.ParameterDirection.Input,SqlDbType=SqlDbType.VarChar },
                        };
                        ...
                        //marketPrices =
                        //   db.MarketPrice.Where(x => x.FileType == 8)
                        //       .OrderBy(x => x.Symbol).Select(x => x.Symbol).Distinct()
                        //       //.Take(50)
                        //       .ToList();
                    }
        ...
        public object GetCloseData(DateTime workingDate, DateTime expiryDate, string exchange, string instrument, string optionType, string symbole, string type, string strickPrice)
        {
            dynamic closeData = null;
            try
            {
                if (exchange == "BSE")
                {
                    if (type == "EQ")
                    {
                        ...
                        {
                            closeData =
                                db.MarketPrice.Where(
                                x => x.UploadDate < workingDate && x.FileType == 5 &&
                                x.ScriptCode == symbole)
                                .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                .FirstOrDefault();
                        }
                    }
                    else if (type == "DERIVATIVE")
                    {
                        if (instrument.Contains("OPT"))
                        {
                            BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                            {
                                closeData =
                                    db.MarketPrice.Where(
                                        x =>
                                            x.UploadDate < workingDate && x.ExpiryDate == expiryDate &&
                                            x.OptionType == optionType && x.FileType == 6 && x.StrikePrice == strickPrice &&
                                            x.Symbol == symbole)
                                        .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                        .FirstOrDefault();
                            }
                        ...
                        else
                        {
                            ...
                            new SqlParameter("@OptionType", "XX"),
                            new SqlParameter("@FileType", 6),
                            new SqlParameter("@Symbol", symbole),
                            new SqlParameter("@StrikePrice", "0")
                            };

                            dt = SqlHelper.ExecuteDataTable(sqlconn, "GetPDLPDHData", paramList);

                            if (dt != null && dt.Rows.Count > 0)
                            {
                                DataRow dr = dt.Rows[0];
                                closeData = dr["Close"].ToString();
                            }
                        }
                ...
                else
                {
                    if (type == "EQ")
                    {
                        BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                        {
                            closeData =
                                db.MarketPrice.Where(
                                x => x.UploadDate < workingDate && x.FileType == 4 &&
                                x.Symbol == symbole)
                                .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                .FirstOrDefault();
                        }
                            ...
                            BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                            {
                                closeData =
                                    db.MarketPrice.Where(
                                        x =>
                                            x.UploadDate <= workingDate && x.ExpiryDate == expiryDate &&
                                            x.OptionType == optionType && x.FileType == 11 && x.StrikePrice == strickPrice &&
                                            x.Symbol == symbole)
                                        .OrderByDescending(x => x.UploadDate).Select(x => x.Close)
                                        .FirstOrDefault();
                            }
        ...
        public List<DateTime?> AutoCompleteexpiry(string compname, string optiontype)
        {
            try
            {
                //connection();
                //List<SelectListItem> items = new List<SelectListItem>();
                //List<DateTime> listDataexpiryMV = null;
                List<DateTime?> expireDate = new List<DateTime?>();
                //String sql = "SELECT distinct[ExpiryDate] FROM [BhagirathFinCare].[dbo].[MarketPrice] where symbol like '" + compname + "' and OptionType like '" + optiontype + "' and Expirydate is not null  order by [ExpiryDate]";
                //SqlCommand cmd = new SqlCommand(sql, conn);
                //SqlDataReader rdr = cmd.ExecuteReader();
                //// used to get number of record returns from query
                //DataTable dt = new DataTable();
                //dt.Load(rdr);
                //int numRows = dt.Rows.Count;
                //DateTime[] arrayData = new DateTime[numRows];
                //// record returns ends here
                //int i = 0;
                ...
                //    {
                //        Text = (DateTime)rdr["ExpiryDate"],
                //        Value = (DateTime)rdr["ExpiryDate"]


                //    });

                //    arrayData[i] = (DateTime)rdr["ExpiryDate"];
                //    i = i + 1;
                //}
                //listDataexpiryMV = arrayData.ToList();

                BhagirathFinCareEntities context = new BhagirathFinCareEntities();

                expireDate =
                                   context.MarketPrice.Where(x => x.Symbol.ToLower() == compname.ToLower().Trim() && x.OptionType == optiontype)
                                       .OrderByDescending(x => x.ExpiryDate)
                                       .Select(x => x.ExpiryDate).Distinct()
                                       .ToList();

                expireDate.Sort((a, b) => Convert.ToDateTime(a).CompareTo(Convert.ToDateTime(b)));
                //var expireDate1 = context.MarketPrice.Where(x => x.Symbol.ToLower() == compname.ToLower().Trim() && x.OptionType == optiontype)
                //                       .Select(x =>new { x.ExpiryDate } ).Distinct()
                //                       .ToList();
                //var expireDate2 = expireDate1.OrderBy(x => x.ExpiryDate).ToList();
                //List<DateTime> expArrayList = new List<DateTime>();
                //foreach (object item in expireDate2)
                //{
                //    expireDate.Add(Convert.ToDateTime(item));
                //}
        ...
        public object GetStrikePrice(string exchange, string type, string symbol, DateTime expireDate)
        {
            BhagirathFinCareEntities db = new BhagirathFinCareEntities();
            ...
            try
            {
                ...
                List<MarketPrice> getstrickPrice = new List<MarketPrice>();
                getstrickPrice = ((from m in db.MarketPrice
                                   where m.Symbol == symbol && m.ExpiryDate == expireDate && m.FileType == (int)fileType && m.StrikePrice != null && m.StrikePrice != "0"
                                   select m).OrderBy(x => x.StrikePrice)).Distinct().ToList();
                var strickPrice = getstrickPrice.Select(x => x.StrikePrice).Distinct().ToList();
            ...
            try
            {
                if (dtoCalculate != null)
                {
                    ...
                    try
                    {
                        ...
                        var pdlpdhData = GetPDLPDHData(dtoCalculate, dtoCalculate.Open, dtoCalculate.Close, Convert.ToDateTime(dtoCalculate.WorkingDate, CultureInfo.InvariantCulture), Companyname, (int)fileType);
                        if (pdlpdhData != null)
                        {
                            multiGainer.txt_d11 = pdlpdhData.PDL.ToString();
                            multiGainer.txt_t11 = pdlpdhData.PDH.ToString();

                            var tdltdhData = GetTDLTDHData(dtoCalculate, pdlpdhData.PDL, pdlpdhData.PDH, pdlpdhData.SelectedDateForPDHAndPDL, Companyname, (int)fileType);
                            if (tdltdhData != null)
                            {
                            ...
                            var cdlcdhData = GetCTLDCTHDData(dtoCalculate, (decimal)idl, (decimal)IDH, Convert.ToDateTime(dtoCalculate.WorkingDate, CultureInfo.InvariantCulture), Companyname, (int)fileType);
        ...
        public object getCalculateDataForEQ(string type, string symbol, string exchange, DateTime workingdate, DateTime expirydate, string close, string instrument, string optionType)
        {
            try
            {
                BhagirathFinCareEntities context = new BhagirathFinCareEntities();
                DateTime WorkingDate = workingdate.Date;
                if (type == "EQ")
                {
                    var GetData = context.RecursiveUpload.Where(x => x.Symbol == symbol && x.FileType == 4 && EntityFunctions.TruncateTime(x.UploadDateTime) == EntityFunctions.TruncateTime(WorkingDate)).OrderByDescending(x => x.FileNo).FirstOrDefault();
                    if (GetData != null)
                    {
                        return GetData;
                    }
                ...
                {
                    object GetData;
                    if (instrument == "FUTIDX" || instrument == "FUTIVX" || instrument == "FUTSTK")
                        GetData = context.RecursiveUpload.Where(x => x.Symbol == symbol && x.FileType == 8 && EntityFunctions.TruncateTime(x.UploadDateTime) == EntityFunctions.TruncateTime(WorkingDate) && EntityFunctions.TruncateTime(x.ExpiryDate) == EntityFunctions.TruncateTime(expirydate) && x.Close == close && x.OptionType == "FUT").OrderByDescending(x => x.FileNo).FirstOrDefault();
                    else
                        GetData = context.RecursiveUpload.Where(x => x.Symbol == symbol && x.FileType == 11 && EntityFunctions.TruncateTime(x.UploadDateTime) == EntityFunctions.TruncateTime(WorkingDate) && EntityFunctions.TruncateTime(x.ExpiryDate) == EntityFunctions.TruncateTime(expirydate) && x.Close == close && x.OptionType == optionType).OrderByDescending(x => x.FileNo).ThenByDescending(x => x.Id).FirstOrDefault();
                    if (GetData != null)
                    {
                        return GetData;
                    }
        ...
        public object GetOpenData(DateTime workingDate, DateTime expiryDate, string exchange, string instrument, string optionType, string symbole, string type, string strickPrice)
        {
            dynamic openData = null;
            try
            {
                if (exchange == "BSE")
                {
                    if (type == "EQ")
                    {
                        BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                        {
                            openData =
                                ...
                                x => x.UploadDate < workingDate && x.FileType == 5 &&
                                x.ScriptCode == symbole)
                                .OrderByDescending(x => x.UploadDate).Select(x => x.Open)
                                .FirstOrDefault();
                        }
                    }
                    else if (type == "DERIVATIVE")
                            ...
                            BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                            {
                                openData =
                                    db.MarketPrice.Where(
                                        x =>
                                            x.UploadDate < workingDate && x.ExpiryDate == expiryDate &&
                                            x.OptionType == optionType && x.FileType == 8 && x.StrikePrice == strickPrice &&
                                            x.Symbol == symbole)
                                        .OrderByDescending(x => x.UploadDate).Select(x => x.Open)
                                        .FirstOrDefault();
                            }
        ...
        public PDLPDHSelectedData GetPDLPDHData(DtoRequestModelForCalculate dtoCalculate, decimal currentOpen, decimal currentClose, DateTime currentDate, string companyName,
            int fileType)
        {
            PDLPDHSelectedData pdlpdhSelectedData = null;
            try
            {
                DataTable dt = new DataTable();
                string sqlQuery;
                //string sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType);

                if (dtoCalculate.Exchange.Equals("BSE", StringComparison.OrdinalIgnoreCase))
                {
                    if (dtoCalculate.Type.Equals("EQ", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where ScriptCode='{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} order by [UploadDate] desc", dtoCalculate.Symbole, currentDate.ToString("yyyy-MM-dd"), fileType);
                        dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                    }
                    else if (dtoCalculate.Type.Equals("DERIVATIVE", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        {
                            //sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} and OptionType='XX' order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType);
                            SqlParameter[] paramList = new SqlParameter[] {
                            new SqlParameter("@ExpiryDate", DateTime.ParseExact(dtoCalculate.ExpiryDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            new SqlParameter("@WorkingDate", currentDate.ToString("yyyy-MM-dd")),
                            new SqlParameter("@OptionType", "XX"),
                            new SqlParameter("@FileType", fileType),
                            new SqlParameter("@Symbol", companyName),
                            new SqlParameter("@StrikePrice", "0")
                            };
                        ...
                        {
                            sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} and OptionType='{3}' and StrikePrice='{4}' and ExpiryDate='{5}' order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType, dtoCalculate.OptionType, dtoCalculate.StrickPrice, dtoCalculate.ExpiryDate);
                            dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                        }
                    }
                }
                else if (dtoCalculate.Exchange.Equals("NSE", StringComparison.OrdinalIgnoreCase))
                {
                    if (dtoCalculate.Type.Equals("EQ", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType);
                ...
                {
                    DataRow dr = dt.Rows[0];
                    pdlpdhSelectedData = new PDLPDHSelectedData();
                    pdlpdhSelectedData.CompanyName = companyName;
                    pdlpdhSelectedData.FileType = fileType;
                    pdlpdhSelectedData.PDH = Convert.ToDecimal(dr["High"]);
                    pdlpdhSelectedData.PDL = Convert.ToDecimal(dr["Low"]);
                    pdlpdhSelectedData.SelectedDateForPDHAndPDL = Convert.ToDateTime(dr["UploadDate"]);
                }
            }
            catch (Exception ex)
            {
        ...
        public TDLTDHSelectedData GetTDLTDHData(DtoRequestModelForCalculate dtoCalculate, decimal pdl, decimal pdh, DateTime pdlpdhDate, string companyName, int fileType)
        {
            TDLTDHSelectedData tdltdhSelectedData = null;
            try
            {
                int NoOfDayOfDataToBeTaken = 45;
                if (ConfigurationManager.AppSettings["NoOfDayOfDataToBeTaken"] != null)
                {
                    NoOfDayOfDataToBeTaken = Convert.ToInt16(ConfigurationManager.AppSettings["NoOfDayOfDataToBeTaken"]);
                }
                //string sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} order by [UploadDate] desc", NoOfDayOfDataToBeTaken, companyName, pdlpdhDate.ToString("yyyy-MM-dd"), fileType);
                //DataTable dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                ...
                if (dtoCalculate.Exchange.Equals("BSE", StringComparison.OrdinalIgnoreCase))
                {
                    if (dtoCalculate.Type.Equals("EQ", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where ScriptCode='{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} order by [UploadDate] desc", NoOfDayOfDataToBeTaken, dtoCalculate.Symbole, pdlpdhDate.ToString("yyyy-MM-dd"), fileType);
                        dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                    }
                    else if (dtoCalculate.Type.Equals("DERIVATIVE", StringComparison.OrdinalIgnoreCase))
                    {
                            ...
                            dtoCalculate.Instrument.Equals("FUTSTK", StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            SqlParameter[] paramList = new SqlParameter[] {
                            new SqlParameter("@ExpiryDate", DateTime.ParseExact(dtoCalculate.ExpiryDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            new SqlParameter("@WorkingDate", DateTime.ParseExact(dtoCalculate.WorkingDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            new SqlParameter("@OptionType", "XX"),
                            new SqlParameter("@FileType", fileType),
                            new SqlParameter("@Symbol", companyName),
                            new SqlParameter("@StrikePrice", "0"),
                            new SqlParameter("@NoOfDayOfDataToBeTaken",NoOfDayOfDataToBeTaken)
                            };

                            dt = SqlHelper.ExecuteDataTable(sqlconn, "GetTDLTDHData", paramList);
                        }
                        else if (dtoCalculate.Instrument.Equals("OPTIDX", StringComparison.OrdinalIgnoreCase) ||
                            dtoCalculate.Instrument.Equals("OPTSTK", StringComparison.OrdinalIgnoreCase))
                        {
                            sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} and OptionType='{4}' and StrikePrice='{5}' and ExpiryDate='{6}' order by [UploadDate] desc", NoOfDayOfDataToBeTaken, companyName, pdlpdhDate.ToString("yyyy-MM-dd"), fileType, dtoCalculate.OptionType, dtoCalculate.StrickPrice, dtoCalculate.ExpiryDate);
                            dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                        }
                    }
                }
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        decimal highValue = Convert.ToDecimal(dr["high"]);
                        decimal lowvalue = Convert.ToDecimal(dr["low"]);

                        if (highValue > pdh || lowvalue < pdl)
                        {
                            tdltdhSelectedData = new TDLTDHSelectedData();
                            tdltdhSelectedData.CompanyName = companyName;
                            tdltdhSelectedData.FileType = fileType;
                            tdltdhSelectedData.TDH = highValue;
                            tdltdhSelectedData.TDL = lowvalue;
                            tdltdhSelectedData.SelectedDateForTDHAndTDL = Convert.ToDateTime(dr["UploadDate"]);
                            break;
                        }
                    }
                }
        ...
        public CTLDCTHDSelectedData GetCTLDCTHDData(DtoRequestModelForCalculate dtoCalculate, decimal currentLow, decimal currentHigh, DateTime currentWorkingDate, string companyName, int fileType)
        {
            CTLDCTHDSelectedData ctldcthdSelectedData = new CTLDCTHDSelectedData();
            ctldcthdSelectedData.CompanyName = companyName;
            ctldcthdSelectedData.FileType = fileType;
            decimal higestHigh = currentHigh;
            decimal higestLow = currentLow;
            try
            {
                DataTable dt = new DataTable();
                int NoOfDayOfDataToBeTaken = 45;
                ...
                if (dtoCalculate.Exchange.Equals("BSE", StringComparison.OrdinalIgnoreCase))
                {
                    if (dtoCalculate.Type.Equals("EQ", StringComparison.OrdinalIgnoreCase))
                    {
                        sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where ScriptCode='{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} order by [UploadDate] desc", NoOfDayOfDataToBeTaken, dtoCalculate.Symbole, currentWorkingDate.ToString("yyyy-MM-dd"), fileType);
                        dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                    }
                    else if (dtoCalculate.Type.Equals("DERIVATIVE", StringComparison.OrdinalIgnoreCase))
                    {
                        if (dtoCalculate.Instrument.Equals("FUTIDX", StringComparison.OrdinalIgnoreCase) ||
                            ...
                            dtoCalculate.Instrument.Equals("FUTSTK", StringComparison.OrdinalIgnoreCase))
                        {
                            //sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} and OptionType='{4}' and ExpiryDate='{5}' order by [UploadDate] desc", NoOfDayOfDataToBeTaken, companyName, currentWorkingDate.ToString("yyyy-MM-dd"), fileType,"XX",dtoCalculate.ExpiryDate);
                            //sqlQuery = string.Format("SELECT top 1 [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{0}' and UploadDate < CONVERT(date,'{1}') and FileType = {2} and OptionType='XX' order by [UploadDate] desc", companyName, currentDate.ToString("yyyy-MM-dd"), fileType);
                            SqlParameter[] paramList = new SqlParameter[] {
                            new SqlParameter("@ExpiryDate", DateTime.ParseExact(dtoCalculate.ExpiryDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            new SqlParameter("@WorkingDate", DateTime.ParseExact(dtoCalculate.WorkingDate,"MM/dd/yyyy", new CultureInfo("en-GB"))),
                            ...
                            dt = SqlHelper.ExecuteDataTable(sqlconn, "GetCTLDCTHDData", paramList);
                        }
                        else if (dtoCalculate.Instrument.Equals("OPTIDX", StringComparison.OrdinalIgnoreCase) ||
                            dtoCalculate.Instrument.Equals("OPTSTK", StringComparison.OrdinalIgnoreCase))
                        {
                            sqlQuery = string.Format("SELECT top {0} [Id],[Symbol],[ExpiryDate],[StrikePrice],[OptionType],[Open],[High],[Low],[Close],[UploadDate],[FileType],[ScriptCode] FROM [BhagirathFinCare].[dbo].[MarketPrice] where Symbol like '{1}' and UploadDate < CONVERT(date,'{2}') and FileType = {3} and OptionType='{4}' and ExpiryDate='{5}' and StrikePrice='{6}' order by [UploadDate] desc", NoOfDayOfDataToBeTaken, companyName, currentWorkingDate.ToString("yyyy-MM-dd"), fileType, dtoCalculate.OptionType, dtoCalculate.ExpiryDate, dtoCalculate.StrickPrice);
                            dt = SqlHelper.ExecuteDataTable(sqlconn, CommandType.Text, sqlQuery);
                        }
                    }
                }
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    //currentHigh = Convert.ToDecimal(dt.Rows[0]["High"]);
                    //currentLow = Convert.ToDecimal(dt.Rows[0]["Low"]);

                    string checkCTfor = string.Empty;
                    DataRow baseRow = null;

                    bool isCTHDFound = false, isCTLDFound = false;
                    decimal baseHigh = currentHigh, baseLow = currentLow, rowHigh = 0, rowLow = 0;

                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        rowHigh = Convert.ToDecimal(dt.Rows[i]["High"]);
                        rowLow = Convert.ToDecimal(dt.Rows[i]["Low"]);

                        if (rowHigh == 0 && rowLow == 0)
                        {
                            break;
                        }

                        if (higestHigh < rowHigh)
                            higestHigh = rowHigh;
                        if (higestLow > rowLow)
                            higestLow = rowLow;
                        if (rowHigh <= currentHigh &&
                            rowLow >= currentLow)
                        {
                            continue;
                        }

                        #region For which we continue High or Low at start
                        if (rowHigh > currentHigh && rowLow < currentLow)
                        {
                            baseRow = dt.Rows[i];
                            currentHigh = rowHigh;
                            currentLow = rowLow;
                        ...
                        else if (string.IsNullOrEmpty(checkCTfor) && rowHigh > currentHigh)
                        {
                            checkCTfor = "High";
                            //currentHigh = rowHigh;
                        }
                        else if (string.IsNullOrEmpty(checkCTfor) && rowLow < currentLow)
                        {
                            checkCTfor = "Low";
                            //currentLow = rowLow;
                        }
                        ...
                        #region set continuous currentHigh or currentLow for latest record
                        if (!string.IsNullOrEmpty(checkCTfor) && checkCTfor.Equals("High", StringComparison.OrdinalIgnoreCase)
                            && rowHigh > currentHigh)
                        {
                            currentHigh = rowHigh;
                            currentLow = rowLow;
                        }
                        else if (!string.IsNullOrEmpty(checkCTfor) && checkCTfor.Equals("Low", StringComparison.OrdinalIgnoreCase)
                            && rowLow < currentLow)
                        {
                            currentLow = rowLow;
                            currentHigh = rowHigh;
                        ...
                        #region check if we found CTHD (if latest record do not continue high) or CTLD (if latest record do not continue low)
                        else if (!string.IsNullOrEmpty(checkCTfor) && checkCTfor.Equals("High", StringComparison.OrdinalIgnoreCase)
                            && rowHigh < currentHigh)
                        {
                            ctldcthdSelectedData.CTHD = currentHigh;
                            isCTHDFound = true;
                            baseRow = dt.Rows[i];
                            if (isCTLDFound)
                                break;
                            else
                            {
                                i--;
                                checkCTfor = "Low";
                                currentLow = Convert.ToDecimal(dt.Rows[i]["Low"]);
                            }
                        }
                        else if (!string.IsNullOrEmpty(checkCTfor) && checkCTfor.Equals("Low", StringComparison.OrdinalIgnoreCase)
                            && rowLow > currentLow)
                        {
                            ctldcthdSelectedData.CTLD = currentLow;
                            isCTLDFound = true;
                            baseRow = dt.Rows[i];
                            if (isCTHDFound)
                                break;
                            else
                            {
                                i--;
                                checkCTfor = "High";
                                currentHigh = Convert.ToDecimal(dt.Rows[i]["High"]);
                            }
                        }
                    ...
                    //if (!isCTHDFound && baseRow != null) 
                    //    ctldcthdSelectedData.CTHD = Convert.ToDecimal(baseRow["High"]);
                    //if (!isCTLDFound && baseRow != null)
                    //    ctldcthdSelectedData.CTLD = Convert.ToDecimal(baseRow["Low"]);
                }
            }
            catch (Exception ex)
            {
            ...
            ///This is worst condition solution.
            if (ctldcthdSelectedData.CTHD == 0)
                ctldcthdSelectedData.CTHD = higestHigh;
            ///This is worst condition solution.
            if (ctldcthdSelectedData.CTLD == 0)
                ctldcthdSelectedData.CTLD = higestLow;
            return ctldcthdSelectedData;
        }
        #endregion

        #region Calculate MST and MSTS
        public object GetMHSTandMSTS(DtoRequestModelForCalculate dtoCalculate)
        {
            try
            {
                connection();
                ...
                BhagirathFinCareEntities context = new BhagirathFinCareEntities();
                DateTime newDateTime = DateTime.Now;
                DateTime uploadDate = DateTime.ParseExact(dtoCalculate.WorkingDate, "MM/dd/yyyy",
                    new CultureInfo("en-US"),
                    DateTimeStyles.None);
                DateTime exp = new DateTime();
                if (dtoCalculate.ExpiryDate != "" && dtoCalculate.ExpiryDate != null)
                {
                    exp = DateTime.ParseExact(dtoCalculate.ExpiryDate, "MM/dd/yyyy",
                    new CultureInfo("en-US"),
                    DateTimeStyles.None);
                }


                var uploadDateMonth = uploadDate.Month;
                var expMonth = exp.Month;
                var Exchange = dtoCalculate.Exchange;
                string type = dtoCalculate.Type;
                var instrument = dtoCalculate.Instrument;
                var optiontype = dtoCalculate.OptionType;
                //FileTypeEnum fileType = FileTypeEnum.BSEEQ;
                int fileType = 0;
                switch (Exchange.ToLower())
                {
                ...
                if (type == "DERIVATIVE")
                {
                    if (instrument.Contains("FUT"))
                    {
                        ...
                        List<decimal> listDecimal2 = new List<decimal>();
                        List<DateTime?> expirylist = AutoCompleteexpiry(Companyname, "XX");
                        List<DateTime> expArrayList = new List<DateTime>();
                        foreach (object item in expirylist)
                        {
                            expArrayList.Add(Convert.ToDateTime(item));
                        }
                        Dictionary<int, DateTime> dict = new Dictionary<int, DateTime>();
                        int i = 0;
                        foreach (DateTime item in expArrayList)
                        {
                            if (item.ToString("dd-MM-yyyy HH:mm:ss") == Convert.ToDateTime(exp).ToString("dd-MM-yyyy HH:mm:ss"))
                            {

                                if (uploadDateMonth == expMonth)
                                {
                                    if (i > 0)
                                    {
                                        dict.Add(0, item);
                                        dict.Add(-1, expArrayList[i - 1]);
                                        dict.Add(1, expArrayList[i + 1]);
                                    }

                                }

                                else if (uploadDateMonth < expMonth)
                                {
                                    if (i > 0)
                                    {
                                        dict.Add(-1, expArrayList[i - 2]);
                                        dict.Add(0, expArrayList[i - 1]);
                                        dict.Add(1, item);
                                    }
                                    if (i == 0)
                                    {
                        ...
                        DateTime zero = (DateTime)dict[0];
                        DateTime one = (DateTime)dict[1];
                        DateTime minusone = (DateTime)dict[-1];
                        var alistDecimal = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX").Select(x => x.High).ToList();
                        listDecimal = alistDecimal.Select(decimal.Parse).ToList();
                        HH = listDecimal.Max();
                        var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.OptionType == "XX" && x.Low != "0").Select(x => x.Low).ToList();
                        listDecimal2 = lowlist.Select(decimal.Parse).ToList();
                        HL = listDecimal2.Min();
                        decimal difference = Math.Abs(Convert.ToDecimal(HH) - Convert.ToDecimal(HL));
                        decimal GR1 = difference * Convert.ToDecimal(0.62);
                        decimal GR2 = difference * Convert.ToDecimal(0.75);
                        decimal HMST = Convert.ToDecimal(HH) - Convert.ToDecimal(GR1);
                        decimal HMSTS = Convert.ToDecimal(HH) - Convert.ToDecimal(GR2);
                        decimal LMST = Convert.ToDecimal(HL) + Convert.ToDecimal(GR1);
                        decimal LMSTS = Convert.ToDecimal(HL) + Convert.ToDecimal(GR2);

                        msts.diff = difference;
                        msts.hh = Convert.ToDecimal(HH);
                        msts.hl = Convert.ToDecimal(HL);
                        msts.diff = difference;
                        msts.gr1 = GR1;
                        msts.gr2 = GR2;
                        msts.hmst = HMST;
                        ...
                        msts.lmst = LMST;
                        msts.lmsts = LMSTS;
                        var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && x.ExpiryDate == exp && x.UploadDate <= uploadDate && x.OptionType == "XX").Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                        var days = data.OrderByDescending(x => x.UploadDate).ToList();
                        if (msts.hmst != null && msts.lmst != null)
                        {
                            foreach (var item in days)
                            {
                                decimal idl = Convert.ToDecimal(item.Low);
                                decimal idh = Convert.ToDecimal(item.High);

                                if (idl <= msts.hmst && msts.hmst <= idh)
                                {
                                    msts.MST = Math.Round((msts.hmst), 2);
                                    msts.MSTS = Math.Round((msts.hmsts), 2);
                                    msts.MST2 = Math.Round((msts.lmst), 2);
                                    msts.MSTS2 = Math.Round((msts.lmsts), 2);
                                    return msts;
                                }
                                else
                                {
                                    if (idl <= msts.lmst && msts.lmst <= idh)
                                    {
                                        msts.MST = Math.Round((msts.lmst), 2);
                                        msts.MSTS = Math.Round((msts.lmsts), 2);
                                        msts.MST2 = Math.Round((msts.hmst), 2);
                                        msts.MSTS2 = Math.Round((msts.hmsts), 2);
                                        return msts;
                                    }
                                }
                            }
                    ...
                    else if (instrument.Contains("OPT"))
                    {
                        ...
                        DateTime one = (DateTime)dict[1];
                        DateTime minusone = (DateTime)dict[-1];
                        var alistDecimal = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.StrikePrice == dtoCalculate.StrickPrice && x.OptionType == dtoCalculate.OptionType).Select(x => x.High).ToList();
                        listDecimal = alistDecimal.Select(decimal.Parse).ToList();
                        HH = listDecimal.Max();
                        var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.StrikePrice == dtoCalculate.StrickPrice && x.OptionType == dtoCalculate.OptionType).Select(x => x.Low).ToList();
                        listDecimal2 = lowlist.Select(decimal.Parse).ToList();
                        HL = listDecimal2.Min();
                        decimal difference = Math.Abs(Convert.ToDecimal(HH) - Convert.ToDecimal(HL));
                        decimal GR1 = difference * Convert.ToDecimal(0.62);
                        ...
                        msts.lmst = LMST;
                        msts.lmsts = LMSTS;
                        var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname && minusone <= x.ExpiryDate && x.ExpiryDate <= one && x.UploadDate <= uploadDate && x.StrikePrice == dtoCalculate.StrickPrice && x.OptionType == dtoCalculate.OptionType).Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                        var days = data.OrderByDescending(x => x.UploadDate).ToList();
                        if (msts.hmst != null && msts.lmst != null)
                        {
                            foreach (var item in days)
                ...
                else
                {
                    List<decimal> listDecimal = new List<decimal>();
                    List<decimal> listDecimal2 = new List<decimal>();
                    var alistDecimal = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname).Select(x => x.High).ToList();
                    listDecimal = alistDecimal.Select(decimal.Parse).ToList();
                    HH = listDecimal.Max();
                    var lowlist = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname).Select(x => x.Low).ToList();
                    listDecimal2 = lowlist.Select(decimal.Parse).ToList();
                    HL = listDecimal2.Min();
                    ...
                    msts.lmst = LMST;
                    msts.lmsts = LMSTS;
                    var data = context.MarketPrice.Where(x => x.FileType == fileType && x.Symbol == Companyname).Select(x => new { x.Low, x.High, x.UploadDate }).ToList();
                    var days = data.OrderByDescending(x => x.UploadDate).ToList();
                    if (msts.hmst != null && msts.lmst != null)
                    {
                        foreach (var item in days)
        ...
        #region ema
        public bool UpdateEmaForEquity(DtoRequestModelForCalculate dtoRequestModelForCalculate)
        {
            bool response = false;
            try
            {
                if (dtoRequestModelForCalculate.Type == "EQ")
                {
                    BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                    DateTime newdatetime = new DateTime();
                    DateTime.TryParseExact(dtoRequestModelForCalculate.WorkingDate, "MM/dd/yyyy",
    new CultureInfo("en-US"),
    DateTimeStyles.None, out newdatetime);
                    MarketPrice marketPrice = db.MarketPrice.Where(
                        x => x.UploadDate < newdatetime && x.FileType == 4 &&
                        x.Symbol == dtoRequestModelForCalculate.Symbole)
                        .OrderByDescending(x => x.UploadDate)
                        .FirstOrDefault();
                    if (marketPrice != null)
                    {
                        marketPrice.EMA13 = dtoRequestModelForCalculate.EMA13;
                        marketPrice.EMA34 = dtoRequestModelForCalculate.EMA34;
                        db.Entry(marketPrice).State = EntityState.Modified;
                        db.SaveChanges();
                        response = true;
                    }
                }
        ...
        public bool UpdateLastDayEmaForEquity()
        {
            bool response = false;
            try
            {
                BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                DateTime newdatetime = new DateTime();
                List<string> symbolList = db.MarketPrice.Where(x => x.EMA13 != null).Select(x => x.Symbol).Distinct().ToList();
                foreach (var item in symbolList)
                {
                    MarketPrice marketPrice = db.MarketPrice.Where(
                    x => x.UploadDate < DateTime.UtcNow && x.FileType == 4 &&
                    x.Symbol == item && (x.EMA13 != null || x.EMA34 != null))
                    .OrderByDescending(x => x.UploadDate)
                    .FirstOrDefault();
                    if (marketPrice != null)
                    {
                        MarketPrice marketPrice1 = db.MarketPrice.Where(
                        x => x.UploadDate < DateTime.UtcNow && x.FileType == 4 &&
                        x.Symbol == item)
                        .OrderByDescending(x => x.UploadDate)
                        .FirstOrDefault();
                        if (marketPrice1 != null)
                        {
                            if (marketPrice.EMA13 != null && marketPrice1.EMA13 == null)
                            {
                                double closewithprevema = Convert.ToDouble(marketPrice1.Close) - Convert.ToDouble(marketPrice.EMA13);
                                double smothingprice = closewithprevema * 0.142;
                                marketPrice1.EMA13 = (smothingprice + Convert.ToDouble(marketPrice.EMA13)).ToString();
                            }
                            if (marketPrice.EMA34 != null && marketPrice1.EMA34 == null)
                            {
                                double closewithprevema = Convert.ToDouble(marketPrice1.Close) - Convert.ToDouble(marketPrice.EMA34);
                                double smothingprice = closewithprevema * 0.057;
                                marketPrice1.EMA34 = (smothingprice + Convert.ToDouble(marketPrice.EMA34)).ToString();
                            }
                            db.Entry(marketPrice1).State = EntityState.Modified;
                            db.SaveChanges();
                            return true;
                        }
                    }
        ...
        public MarketPrice getema()
        {
            MarketPrice marketPrice = new MarketPrice();
            try
            {
                BhagirathFinCareEntities db = new BhagirathFinCareEntities();
                marketPrice = db.MarketPrice.Where(
                       x => x.UploadDate < DateTime.UtcNow && x.FileType == 4 &&
                       x.Symbol.Contains("RELIANCE") && (x.EMA13 != null || x.EMA34 != null))
                       .OrderByDescending(x => x.UploadDate)
                       .FirstOrDefault();
                if (marketPrice != null)
                {
